...
- payload = {
            'id': id_receive,
            'exp': datetime.datetime.utcnow() + datetime.timedelta(seconds=5)    <------- ID와 사용시간을 payload에 담고 시크릿키와 합쳐 토큰을 발행하여 유저에게 지급
        }
        token = jwt.encode(payload, SECRET_KEY, algorithm='HS256')

- payload = jwt.decode(token_receive, SECRET_KEY, algorithms=['HS256'])
  user_info = db.users.find_one({"username": payload["id"]})                      <------- payload값을 보기 위해선 복호화에 열쇠인 시크릿 키('SPARTA')라는 것을 이용해서 복호화를 한다..
